...
from noir.application.workflow_service import _execute_finish_steps, finish
...
from noir.domain.models import AuditEvent, BuildResult, ChangePlan, JobInfo, PatchOperation, PatchSet, PlanFileChange, ProjectInfo
from noir.infrastructure.database.engine import init_db
from noir.infrastructure.database.repositories import BuildRepository, EventRepository, JobRepository, ProjectRepository
...
def test_workflow_pipeline_missing_signed_apk_raises_clear_error(pipeline_setup, monkeypatch):
    ...
    plan = ChangePlan(
        ...
        user_request="Test",
        file_changes=[PlanFileChange(relative_path="AndroidManifest.xml", operation=PatchOperationType.MANIFEST_UPDATE)],
    )
    ...
    with pytest.raises(ValueError, match="Signed APK artifact not found"):
        _execute_finish_steps(cfg, job, project_id=project_id, plan=plan, patch=patch, user_id="test_user")
