...
from noir.domain.enums import PatchOperationType, Provenance, WorkflowStage
...
def authorized_workspace_user(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
...
def test_prepare_with_auto_build_executes_rebuild_and_signing(workflow_setup, monkeypatch):
    ...
    monkeypatch.setattr(
        "noir.application.workflow_service.generate_patch", lambda *args, **kwargs: patch
    )
    ...
    monkeypatch.setattr(
        "noir.infrastructure.android_tools.tools.verify_signature",
        lambda config, path: {"verified": True},
    )
