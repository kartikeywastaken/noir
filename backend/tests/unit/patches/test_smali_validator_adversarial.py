...
def test_invoke_parameter_count_mismatch_static():
    ...
    assert any(
        d.error_code in ("ARGUMENT_COUNT_MISMATCH", "PARAMETER_COUNT_MISMATCH")
...
def test_invoke_range_reversed_registers():
    ...
    assert result.is_valid is False
    assert any(
        d.error_code in ("INVALID_REGISTER_RANGE", "ILLEGAL_OPERAND") for d in result.diagnostics
    )
...
def test_move_result_without_preceding_invoke():
    ...
    assert result.is_valid is False
    assert any(
        d.error_code in ("ILLEGAL_MOVE_RESULT", "ILLEGAL_OPERAND") for d in result.diagnostics
    )
...
def test_const_4_overflow():
    ...
    assert result.is_valid is False
    assert any(
        d.error_code in ("LITERAL_OUT_OF_RANGE", "ILLEGAL_OPERAND") for d in result.diagnostics
    )
...
def test_array_of_void_descriptor_rejected():
    ...
    assert result.is_valid is False
    assert any(
        d.error_code in ("INVALID_TYPE_DESCRIPTOR", "ILLEGAL_OPERAND") for d in result.diagnostics
    )
