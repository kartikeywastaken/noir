...
_METHOD_SIG_RE = re.compile(r"^([a-zA-Z0-9_\$<>]+)\(([^)]*)\)(.*)$")
...
class SmaliBytecodeValidator:
    ...
    def validate(
        ...
        *,
        ...
        move_result_labels: dict[str, str] = {}  # label -> move-result opcode
        pending_labels: list[str] = []  # labels defined immediately prior to current instruction
        ...
        for line_idx, line in enumerate(lines, start=1):
            ...
            if code_line.startswith(".end method"):
                ...
                for lbl, lbl_line_num, lbl_line_content in labels_referenced:
                    ...
                    elif lbl in move_result_labels:
                        mr_op = move_result_labels[lbl]
                        diagnostics.append(
                            SmaliDiagnostic(
                                line_number=lbl_line_num,
                                line_content=lbl_line_content,
                                message=(
                                    f"Illegal control flow: branch targets label '{lbl}' which immediately "
                                    f"precedes '{mr_op}'. Dalvik/ART verifier requires move-result* to be "
                                    "reached solely via fall-through from its producing invoke or filled-new-array."
                                ),
                                error_code="ILLEGAL_MOVE_RESULT_BRANCH_TARGET",
                            )
                        )
                in_method = False
            ...
            if code_line.startswith(".annotation") or re.search(
                r"(?:^|\s)\.subannotation\b", code_line
            ):
                annotation_depth += 1
                continue
            ...
            if code_line.startswith("."):
                dir_name = (
                    " ".join(code_line.split()[:2])
                    if code_line.startswith(".end ")
                    else code_line.split()[0]
                )
            ...
            # If move-result*, record any labels immediately preceding it
            first_word = code_line.split(None, 1)[0]
            if first_word in {"move-result", "move-result-wide", "move-result-object"}:
                for pending_lbl in pending_labels:
                    move_result_labels[pending_lbl] = first_word
    ...
    def _validate_instruction(
        ...
        if opcode in _DALVIK_ZERO_ARG_OPCODES:
            ...
            if opcode == "return-void" and method_sig:
                cls._check_return_type_match(opcode, line_idx, raw_line, diagnostics, method_sig)
            return None
        ...
        if opcode in _NUMERIC_CONST_OPCODES:
            ...
            if len(ops) != 2:
                ...
                lit_to_parse = (
                    lit_str.rstrip("Ll")
                    if (is_wide_op or lit_str.endswith(("L", "l")))
                    else lit_str
                )
                val: int | None = None
                try:
        ...
        if is_lit8 or is_lit16:
            ...
            if len(ops) != 3:
                ...
                try:
                    lit_val = (
                        int(lit_str, 16)
                        if lit_str.startswith(("0x", "-0x", "+0x"))
                        else int(lit_str, 10)
                    )
        ...
        if (
            ...
            if len(ops) != 2:
                ...
                is_wide_0 = False
                is_wide_1 = False
                if opcode.startswith("move-wide") or opcode in {
                    "neg-long",
                    "not-long",
                    "neg-double",
                    "long-to-double",
    ...
    def _validate_move_result(
        ...
        elif opcode == "move-result-object" and not (
            inv_ret.startswith("L") or inv_ret.startswith("[")
        ):
            diagnostics.append(
                SmaliDiagnostic(
                    line_number=line_idx,
        ...
        elif opcode == "move-result-wide" and inv_ret not in ("J", "D"):
            diagnostics.append(
                SmaliDiagnostic(
                    line_number=line_idx,
                    line_content=raw_line,
    ...
    def _validate_invoke_instruction(
        ...
        if any(
            code_line.startswith(pfx) for pfx in ("invoke-polymorphic", "invoke-polymorphic/range")
        ):
        ...
        if m_name == "<init>":
            allowed_invoke = any(
                code_line.startswith(pfx) for pfx in ("invoke-direct", "invoke-super")
            )
        ...
        if is_standard_invoke and not any(
            d.error_code == "INVALID_PARAM_DESCRIPTOR" for d in diagnostics
        ):
            expected_arg_regs = (0 if is_invoke_static else 1) + sum(
                _param_register_count(p) for p in param_types
            )
            if is_range:
                ...
                toks = (
                    [t.strip() for t in reg_list_str.split(",") if t.strip()]
                    if reg_list_str
                    else []
                )
    ...
    def _validate_filled_new_array(
        ...
        if not array_type.startswith("[") or not _is_valid_type_descriptor(
            array_type, allow_void=False
        ):
            diagnostics.append(
                SmaliDiagnostic(
                    line_number=line_idx,
    ...
    def _check_register(
        ...
        if not m:
            error_code = (
                "INVALID_REGISTER_NAME" if re.match(r"^[a-zA-Z]\d+$", reg) else "INVALID_REGISTER"
            )
            diagnostics.append(
                SmaliDiagnostic(
                    line_number=line_idx,
        ...
        if (
            declared_registers is not None
            and reg_type == "v"
            and (reg_num + regs_needed - 1) >= declared_registers
        ):
            msg = (
        ...
        if (
            reg_type == "p"
            and param_reg_count is not None
            and (reg_num + regs_needed - 1) >= param_reg_count
        ):
            msg = (
