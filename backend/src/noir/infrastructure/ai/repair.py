...
def repair_smali_operation(
    ...
    *,
        ...
        try:
            ...
            if attempt == max_retries:
                raise SmaliRepairError(
                    f"Model call failed during Smali self-repair: {exc}"
                ) from exc
            continue
    ...
    logger.error(
        "Smali self-repair failed after %d attempts. Summary: %s", max_retries, error_summary
    )
    raise SmaliRepairError(
...
def repair_patch_set(
    ...
    *,
    ...
    for idx, op in enumerate(patch.operations):
        is_smali_op = op.operation.value.startswith("smali_") or (
            op.relative_path.endswith(".smali") and bool(op.new_content)
        )
