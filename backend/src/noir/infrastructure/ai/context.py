...
class AiContextTools:
    ...
    def _discover_primary_entrypoints(self) -> set[str]:
        ...
        if self.analysis:
            application_class = getattr(self.analysis, "application_class", None)
            if application_class:
                app_class_names.add(application_class)
        ...
        if manifest_path.is_file():
            try:
                ...
                logger.debug("Structured manifest entrypoint discovery failed", exc_info=True)
            ...
            try:
                ...
                logger.debug("Direct manifest entrypoint discovery failed", exc_info=True)
