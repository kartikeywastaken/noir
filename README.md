...
applies the same patch twice. Upload sessions and offsets persist for 24 hours; socket failures
are retried with bounded backoff while the selected file remains available. Unknown download
lengths show bytes without an invented percentage. Android still controls its native save-file
confirmation.
